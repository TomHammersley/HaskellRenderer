...
renderLine :: Direction -> IO ()
renderLine (Vector x y z _) = do
  glVertex3f (1.1 * (realToFrac x)) (1.1 * (realToFrac y)) (1.1 * (realToFrac z))
  glVertex3f (realToFrac x) (realToFrac y) (realToFrac z)

hemispherePoints :: [Direction]
hemispherePoints = fst $ generateDirectionsOnHemisphereUnstratified 512 1 (pureMT 12345)
...
drawScene = do
  ...
  -- Render my test data
  glBegin gl_LINES
  _ <- mapM renderLine hemispherePoints
  glEnd
...
main = do
     ...
     GLFW.setWindowTitle "Hemisphere direction generation test"
